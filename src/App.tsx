        ...
        <div className="below-grid">
          ...
          {isFrame && <>
            <div className="bg-columns">
              ...
              <div className={sameAsA ? 'dimmed' : ''}><FieldLabel>Colour B</FieldLabel><div className="background-swatches">{backgrounds.map(bg => <button key={bg} disabled={sameAsA} className={colourB === bg ? 'active' : ''} style={{ background: bg }} onClick={() => apply({ frameBackground2: bg })}/>)}</div><div className="custom-bg"><Palette size={14}/><span>Custom</span><input type="color" disabled={sameAsA} value={isHex(colourB) ? colourB : '#30382a'} onChange={e => apply({ frameBackground2: e.target.value })}/></div><label className="check-label dark"><input type="checkbox" checked={sameAsA} onChange={e => apply(e.target.checked ? { frameBackground2: undefined } : { frameBackground2: isHex(draft.frameBackground) ? draft.frameBackground.toLowerCase() : '#30382a', frameTransition: draft.frameTransition || 'Fade', frameTransitionTime: draft.frameTransitionTime || 1, frameTransitionStart: draft.frameTransitionStart ?? Math.max(0, (draft.duration - 1) / 2) })}/><span><Check size={11}/></span>Same as A</label>{!sameAsA && !change && <small className="colour-b-hint">B starts as a copy of A. Pick another colour for the A → B change.</small>}</div>
            </div>
