...
function App() {
  ...
  const applyBulkTransition = (random = false) => {
    ...
    const withParams = random && randomizeParams ? ' · transition parameters randomized' : ''
  ...
  // Re-roll every parameter of the existing transitions without changing
  // their names or durations. This includes easing/reverse for native xfade
  // transitions and the registry-defined values for GL transitions.
  const randomizeBulkParams = () => {
    const eligible = media.slice(0, -1).map(x => x.id)
    const ids = selectedTransitions.length ? selectedTransitions : eligible
    if (!ids.length) { notify(selectedTransitions.length ? 'No transitions in the selection' : 'No transitions in the storyline yet'); return }
    setMedia(items => items.map(item => ids.includes(item.id) ? { ...item, ...randomTransitionSettings(item.transition) } : item))
    notify(`Transition parameters randomized on ${ids.length} transition${ids.length === 1 ? '' : 's'}`)
  }
  const applyDuration = () => {
