...
from .font_registry import font_files
from .text_effects import LayeredText, build_text_overlay, normalize_text_window, overlay_plan, plan_engine
from .text_motion import BgChange, build_text_motion_overlay, has_stack, stack_fallback_fields
...
# Single source: registry/fonts.json (generated by scripts/build_fonts.py), the
# same catalogue the editor's font picker reads (src/fonts.ts).
FONT_FILES: dict[str, dict[str, str]] = font_files()
...
class Renderer:
    ...
    def _text_filter(self, item: dict[str, Any], defaults: dict[str, Any], width: int, height: int, ass_path: Path | None = None, lead_in: float = 0.0,
        ...
        if has_stack(item):
            # Stacked effects (textFx): the channel compositor in text_motion.py.
            # It needs libass; without it the caption keeps its words, position
            ...
            if has_ass and ass_path is not None:
                ...
                if change is not None:
                    bg = BgChange(
                        ...
                        transition=self.resolve_xfade(change["transition"]),
                        start=max(0.0, lead_in) + change["start"], time=change["time"],
                    )
                return build_text_motion_overlay(item, defaults, width, height, layer_fps, item["textStart"], item["textEnd"],
                                                 self.settings.fonts_dir, ass_path, bg)
            log.warning("FFmpeg build lacks the libass 'ass' filter; stacked text effects degrade to fades")
            item = {**item, **stack_fallback_fields(item)}
            item.pop("textFx", None)
